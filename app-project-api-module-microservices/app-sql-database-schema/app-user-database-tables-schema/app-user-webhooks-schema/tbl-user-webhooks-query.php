...
?>
<?php
class TblUserWebhooksQuery extends TblUserWebhooks {
    ...
    private function getSQLiteColumnProperty() {
        $tablePropertyList = array(
            ...
            $this->event_type       => "TEXT NOT NULL CHECK(event_type IN ('login', 'login_failed'))",
            $this->status => "TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('pending', 'delivered', 'failed'))",
            $this->created_at      => "TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP",
