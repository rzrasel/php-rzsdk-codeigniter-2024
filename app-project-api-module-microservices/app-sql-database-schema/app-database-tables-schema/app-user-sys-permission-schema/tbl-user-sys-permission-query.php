...
?>
<?php
class TblUserSysPermissionQuery extends TblUserSysPermission {
    ...
    private function getSQLiteColumnProperty() {
        $tablePropertyList = array(
            ...
            $this->user_type        => "TEXT NOT NULL DEFAULT 'registered' CHECK(user_type IN ('open', 'guest', 'registered', 'admin')), -- User type for this permission",
            $this->scope            => "TEXT NOT NULL DEFAULT 'global' CHECK(scope IN ('global', 'organization', 'team', 'user')), -- Permission scope",
            $this->is_public        => "BOOLEAN NOT NULL DEFAULT FALSE, -- Is this permission accessible to all user types?",
