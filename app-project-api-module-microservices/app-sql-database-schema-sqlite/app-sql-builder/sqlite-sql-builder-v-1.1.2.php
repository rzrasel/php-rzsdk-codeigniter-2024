...
class SqliteSqlBuilder {
    ...
    private array $createTableList = array();
    private array $dropTableList = array();
    //
    ...
    private function buildSchemaSql(DatabaseSchemaModel $schema): string {
        ...
        //DebugLog::log($this->dropTableList);
    ...
    private function buildKeySql(DatabaseSchemaModel $schema, TableDataModel $table, ColumnKeyModel $key): string {
        ...
        if(!empty($referenceColumnInfo)) {
            ...
            $this->onRearrangeCreateTableList($workingTableName, $referenceTableName);
    ...
    private function onRearrangeCreateTableList($mainTable, $referenceTable) {
        if(empty($mainTable) || empty($referenceTable)) {
            return;
        }
        $workingIndex = -1;
        $referenceIndex = -1;
        $arrayKeys = array_keys($this->createTableList);
        for($i = 0; $i < count($arrayKeys); $i++) {
            if($arrayKeys[$i] === $mainTable) {
                $workingIndex = $i;
            }
            if($arrayKeys[$i] === $referenceTable) {
                $referenceIndex = $i;
            }
        }
